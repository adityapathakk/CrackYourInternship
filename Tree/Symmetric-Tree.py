...
class Solution:
    ...
    def isSame(self, leftRoot, rightRoot):
        if leftRoot == None and rightRoot == None: # if both root nodes are null nodes, we return true
            return True
        if leftRoot == None or rightRoot == None: # if only one of them is null, we return false
            return False
